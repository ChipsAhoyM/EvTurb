...
def get_args():
    parser = argparse.ArgumentParser(description='EvTurb end-to-end training')
...
def train(model, train_loader, optimizer, epoch, args, device, per_loss, writer):
    model.train()
...
def validate(model, val_loader, epoch, args, device, per_loss, writer):
    model.eval()
    criterion_mse = nn.MSELoss()
    val_loss = 0.0
    val_psnr = 0.0
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        train(model, train_loader, optimizer, epoch, args, device, per_loss, writer)
        val_loss, val_psnr = validate(model, val_loader, epoch, args, device, per_loss, writer)
        scheduler.step()

        if val_psnr > best_val_psnr:
            best_val_psnr = val_psnr
            save_model(model, optimizer, scheduler, epoch + 1,
